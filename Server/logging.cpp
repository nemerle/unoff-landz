...
void write_to_file(char *file_name, char *text) {
    ...
    **/
    ...
    if((file=fopen(file_name, "a"))==nullptr) {
...
void clear_file(const char *file_name){
    ...
    **/
    ...
    if((file=fopen(file_name, "a"))!=nullptr){
        ...
        if((file=fopen(file_name, "w"))==nullptr){
...
void log_text(int event_type, const char *fmt, ...){
...
void log_event(int event_type, const char *fmt, ...){
    ...
    get_date_stamp_str(time(nullptr), date_stamp_str);
...
void initialise_logs(){
    ...
    //open the file in append mode so we can detect if there's an existing file
    if((file=fopen(INITIALISATION_LOG_FILE_NAME, "a"))!=nullptr){

        //if there's an existing file, close and reopen it in write mode (has the effect of clearing the existing content)
        fclose(file);

        if((file=fopen(INITIALISATION_LOG_FILE_NAME, "w"))==nullptr){
...
void log_sqlite_error(const char *error_type, const char *function_name, const char *module_name, int line_number, int return_code, const char *sql_stmt){
        ...
        log_text(EVENT_ERROR, "sqlite error [%s]",sqlite3_errmsg(db));
        switch(return_code) {
